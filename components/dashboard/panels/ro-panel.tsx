...
export function ROPanel() {
  ...
  return (
    <div className="space-y-2.5 sm:space-y-3 md:space-y-5 p-2 sm:p-3 md:p-6 min-h-screen w-full max-w-full overflow-x-hidden">
      ...
      {/* Recent Nominations */}
      <Card className="bg-white border-0 shadow-sm rounded-xl">
        <CardHeader className="pb-1 sm:pb-1.5 md:pb-2 px-2 sm:px-3 md:px-4 pt-2 sm:pt-3 md:pt-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 sm:gap-2 md:gap-3">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <div className="p-0.5 sm:p-1 md:p-1.5 bg-emerald-100 rounded-lg">
                <ClipboardList className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 text-emerald-600" />
              </div>
              <CardTitle className="text-[9px] sm:text-[10px] md:text-sm font-semibold text-slate-800">
                Recent Nominations
              </CardTitle>
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2">
              <div className="relative flex-1 sm:flex-none">
                <Search className="absolute left-1.5 sm:left-2 top-1/2 -translate-y-1/2 h-2.5 w-2.5 sm:h-3 sm:w-3 text-slate-400" />
                <Input
                  placeholder="Search..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="pl-5 sm:pl-6 md:pl-7 h-6 sm:h-7 md:h-8 w-full sm:w-24 md:w-40 text-[9px] sm:text-[10px] md:text-xs"
                />
              </div>
              <Select value={wardFilter} onValueChange={setWardFilter}>
                <SelectTrigger className="h-6 sm:h-7 md:h-8 w-20 sm:w-24 md:w-32 text-[9px] sm:text-[10px] md:text-xs">
          ...
          <div className="space-y-0.5 sm:space-y-1 md:space-y-2">
            {getFilteredNominations().length > 0 ? (
              getFilteredNominations().map((nomination) => (
                <div
                  key={nomination.id}
                  className="flex items-center justify-between p-1.5 sm:p-2 md:p-3 rounded-lg bg-slate-50 hover:bg-slate-100 transition-colors gap-1.5 sm:gap-2"
                >
                  <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 min-w-0">
                    <div className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 rounded-full bg-indigo-100 flex items-center justify-center shrink-0">
                      <User className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 text-indigo-600" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-[9px] sm:text-[10px] md:text-sm font-medium text-slate-800 truncate">
                        {nomination.candidateName}
                      </p>
                      <p className="text-[7px] sm:text-[8px] md:text-xs text-slate-500 truncate">
                  ...
                  <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 shrink-0">
                    ...
                    <span className="text-[7px] sm:text-[8px] md:text-xs text-slate-400 hidden sm:inline whitespace-nowrap">
              ...
              <div className="text-center py-4 sm:py-5 md:py-8 text-slate-400 text-[10px] sm:text-xs md:text-sm">
