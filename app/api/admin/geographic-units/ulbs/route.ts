// ULBs API - Full CRUD operations with hierarchical filtering
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      status: "success",
      message: "ULBs retrieved successfully",
      data: result.data,
      pagination: result.pagination
    });
  } catch (error) {
    console.error("ULBs GET error:", error);
    return NextResponse.json(
      { 
        status: "error",
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = createULBSchema.parse(body);

    try {
      const ulb = await createULB(data);
    ...
    console.error("ULB POST error:", error);
    return NextResponse.json(
      { 
        status: "error",
        message: "Internal server error",
